...
# Use a DEDICATED throwaway table, NOT the real Gaia.Source that RunChallenge /
# the web UI read from — otherwise running the tests wipes the demo data.
TEST_TABLE = "Gaia.SourceTest"
...
def _setup(conn):
    ...
    cur.execute(f'CREATE TABLE {TEST_TABLE} (source_id BIGINT, ra DOUBLE, "dec" DOUBLE, '
                f"phot_g_n_obs INTEGER, phot_g_mean_flux DOUBLE, "
                f"phot_g_mean_flux_error DOUBLE, phot_g_mean_mag DOUBLE)")
    cur.executemany(f"INSERT INTO {TEST_TABLE} VALUES (?,?,?,?,?,?,?)", ROWS)
    conn.commit()
...
def test_sql_matches_python_reference(iris_conn):
    ...
    try:
        ...
        cur.execute(build_query(10, table=TEST_TABLE))
        got = cur.fetchall()
        ...
        for sid, ra, dec, n_obs, flux, flux_err, mag in ROWS:
            ...
            if ref["pct_change"] >= 10:
                ...
                # IRIS DB-API returns computed columns as strings
...
def test_results_sorted_desc(iris_conn):
    ...
    try:
        cur = iris_conn.cursor()
        cur.execute(build_query(0, table=TEST_TABLE))
        # coerce to float for correct numeric comparison
